...
public class RemoveProductFromCardAction implements Action {
    private static final Logger log = LoggerFactory.getLogger(RemoveProductFromCardAction.class);
    ...
    public ActionResult execute(HttpServletRequest req, HttpServletResponse resp) throws ActionException, IOException {
        ...
        if (null != user) {
            ...
            if (integer < 2)
                try {
                    service.removeFromOrder(order, product1);
                } catch (ServiceException e) {
                ...
                order.setProducts(Collections.singletonMap(product1, --integer));
                try {
